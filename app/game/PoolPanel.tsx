...
export default function PoolPanel({pool,names,onClaim}: {pool:Pool;names:string[];onBet?:(kart:number,amount:number)=>void;onClaim:()=>void;onCollect?:()=>void;onCancel?:()=>void}) {
  ...
  return <div className="participation-panel"><section className="pool-panel" aria-label="Pepecoin Kart race lobby">
    ...
    <p className="pool-state is-open">Kekspace Circuit</p><p className="small-note">Choose a racer and join the next four-kart grid. Everyone else watches.</p>
    <div className="bet-racers">{names.map((name,kart)=><div key={kart} className={pool.slots[kart]?"is-selected":""}>{RACERS[kart]&&<img src={RACERS[kart].portrait} alt="" width="42" height="42"/>}<span>{name}</span><small>{pool.slots[kart]?pool.slots[kart]==="you"?"YOU":"BOT":"OPEN"}</small></div>)}</div>
